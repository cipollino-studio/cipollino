...
use crate::{editor::{selection::Selection, EditorState}, project::{action::Action, frame::Frame, layer::Layer, obj::ObjPtr}};

use super::{FrameGridRow, TimelinePanel};

impl FrameGridRow {

    fn frame_area_layer(&self, timeline: &mut TimelinePanel, ui: &mut egui::Ui, frame_w: f32, frame_h: f32, state: &mut EditorState, rect: egui::Rect, response: &egui::Response, mouse_went_down: bool, layer_ptr: ObjPtr<Layer>) -> Option<()> {
    ...
    fn frame_area(&self, timeline: &mut TimelinePanel, ui: &mut egui::Ui, frame_w: f32, frame_h: f32, state: &mut EditorState, response: &egui::Response, mouse_went_down: bool, rect: egui::Rect) {
        match &self.kind {
            super::FrameGridRowKind::Layer(layer) => self.frame_area_layer(timeline, ui, frame_w, frame_h, state, rect, response, mouse_went_down, *layer)
        };
    }
...
pub fn frames(timeline: &mut TimelinePanel, ui: &mut egui::Ui, frame_w: f32, frame_h: f32, state: &mut EditorState, n_frames: i32, grid_rows: &Vec<FrameGridRow>) {
    ...
    if response.drag_released() {
        let action = std::mem::replace(&mut timeline.frame_drag_action, None);
